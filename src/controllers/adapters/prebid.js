  ...
  if (util.isOwnProperty(kgpvMap, responseID)) {
    ...
    */
    ...
    if (pbBid.bidderCode && CONFIG.isServerSideAdapter(pbBid.bidderCode)) {
      var divID = kgpvMap[responseID].divID;
      var temp1 = getPBCodeWithWidthAndHeight(divID, pbBid.bidderCode, pbBid.width, pbBid.height);
      var temp2 = getPBCodeWithoutWidthAndHeight(divID, pbBid.bidderCode);
    ...
    /* istanbul ignore else */
    if (pbBid.bidderCode) {
      var sessionBids = JSON.parse(sessionStorage.getItem('openWrapBids')) || {};
      // var divId = kgpvMap[responseID].divID;
      var bidObj = transformPBBidToOWBid(pbBid, kgpvMap[responseID].kgpv);

      if (sessionBids[bidObj.kgpv]) {
        sessionBids[bidObj.kgpv].push(bidObj);
      } else {
        sessionBids[bidObj.kgpv] = [bidObj];
      }

      sessionStorage.setItem('openWrapBids', JSON.stringify(sessionBids));

      console.log(bidObj.kgpv, sessionBids);
      bidManager.setBidFromBidder(
        kgpvMap[responseID].divID,
        transformPBBidToOWBid(pbBid, kgpvMap[responseID].kgpv)
      );
    }
  } else {
  ...
  for (var responseID in bidResponses) {
    ...
    if (util.isOwnProperty(bidResponses, responseID) && util.isOwnProperty(kgpvMap, responseID)) {
      ...
      for (var i = 0; i < bids.length; i++) {
        ...
        /* istanbul ignore else */
        if (bid.bidderCode) {
          bidManager.setBidFromBidder(kgpvMap[responseID].divID, transformPBBidToOWBid(bid, kgpvMap[responseID].kgpv));
        }
      }
    }
  }
...
function generatedKeyCallback(adapterID, adUnits, adapterConfig, impressionID, generatedKey, kgpConsistsWidthAndHeight, currentSlot, keyConfig, currentWidth, currentHeight) {
  var code;
  var sizes;
  var divID = currentSlot.getDivID();
...
function fetchBids(activeSlots, impressionID) {
  window.pwtCreatePrebidNamespace('pbjs');
  ...
  if (util.isFunction(window.pbjs.onEvent)) {
    window.pbjs.onEvent('bidResponse', pbBidStreamHandler);
  } else {
    util.log('PreBid js onEvent method is not available');
    return;
  }

  window.pbjs.logging = util.isDebugLogEnabled();
  ...
  if (adUnitsArray.length > 0 && window.pbjs) {
    try {
      ...
      if (util.isFunction(window.pbjs.setConfig)) {
